...
fn main() {
    ...
    let (mut rl, thread) = raylib::init()
        ...
        .title("OFF (The 3D version) - Fixed")
        .build();
    ...
    while !rl.window_should_close() {
        ...
        if let GameState::Menu = state {
            ...
            if key_enter {
                ...
                // Recargar enemigos y workers del nuevo nivel
                ...
                println!("Nivel {}: {} enemigos, {} workers", current_level + 1, enemies.len(), workers.len());
        ...
        // Actualizar enemigos (ahora con detección de jugador)
        ...
        // Dibujar enemigos (solo si no están bloqueados por paredes)
        ...
        // Dibujar workers (T) - solo si no están bloqueados
        ...
        // Dibujar workers en minimap
        mini_fb.set_current_color(Color::SKYBLUE);
        for w in workers.iter() {
            let px = w.pos.x * (mini_fb.width as f32 / (maze[0].len() as f32 * block_size as f32));
            let py = w.pos.y * (mini_fb.height as f32 / (maze.len() as f32 * block_size as f32));
            mini_fb.draw_rect(px as i32 - 2, py as i32 - 2, 4, 4);
        }
        ...
        d.draw_text(&format!("Enemies: {} | Workers: {}", enemies.len(), workers.len()), 10, 100, 18, Color::WHITE);
